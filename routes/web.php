...
Route::middleware(['auth'])->group(function () {
    Route::get('/ganti-password-awal', [ProfileController::class, 'showChangePassword'])->name('password.change.form');
    Route::post('/ganti-password-awal', [ProfileController::class, 'processChangePassword'])->name('password.change.process');
});

Route::middleware(['auth', 'role:admin', 'must.change.password'])->group(function () {
    Route::get('/admin/dashboard', [AdminDashboardController::class, 'index'])->name('admin.dashboard');
    Route::resource('/admin/guru-bk', GuruBKController::class)->names('admin.guru-bk');
    Route::resource('/admin/siswa', SiswaController::class)->names('admin.siswa');

    // Fix X2: pindah arsip ke dalam grup auth+admin
    ...
    // Fix X2: pindah nonaktifkan-kelas ke dalam grup auth+admin
    ...
    Route::get('/admin/import-siswa', [ImportSiswaController::class, 'index'])->name('admin.import-siswa.index');
    Route::post('/admin/import-siswa', [ImportSiswaController::class, 'store'])->name('admin.import-siswa.store');
    Route::get('/admin/import-siswa/template', [ImportSiswaController::class, 'downloadTemplate'])->name('admin.import-siswa.template');
    Route::get('/admin/import-siswa/download-hasil/{id}', [ImportSiswaController::class, 'downloadHasil'])->name('admin.import-siswa.download-hasil');

    // Download data siswa
    Route::get('/admin/siswa-download', [SiswaController::class, 'download'])->name('admin.siswa.download');

    // Periode wajib update data
...
Route::middleware(['auth', 'role:bk', 'must.change.password'])->group(function () {
    ...
    Route::resource('/bk/laporan', LaporanController::class)->names('bk.laporan');
    ...
    // Download BK
    Route::get('/bk/download/kasus/{id}', [DownloadController::class, 'downloadKasusPdf'])->name('bk.download.kasus');
    Route::get('/bk/download/semua-pdf', [DownloadController::class, 'downloadSemuaPdf'])->name('bk.download.semua-pdf');
    Route::get('/bk/download/semua-excel', [DownloadController::class, 'downloadSemuaExcel'])->name('bk.download.semua-excel');
});
...
// Halaman wajib update data (dengan periode aktif, tanpa blokir akses halaman itu sendiri)
Route::middleware(['auth', 'role:orang_tua', 'must.change.password'])->group(function () {
    Route::get('/orang-tua/update-data', [ProfileController::class, 'editSiswa'])->name('orang_tua.update-data');
    Route::patch('/orang-tua/update-data', [ProfileController::class, 'updateSiswa'])->name('orang_tua.update-data.save');
});

Route::get('/panduan', fn() => view('panduan.index'))->middleware(['auth', 'must.change.password']);
