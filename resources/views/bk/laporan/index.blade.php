...
@section('content')
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Laporan Permasalahan</h1>
            <p class="text-slate-500 mt-1 text-sm">Kelola semua laporan masuk dari orang tua.</p>
        </div>
    ...
    <section class="mb-8">
        <div class="flex items-center gap-2 mb-4">
            ...
            <h2 class="text-sm font-bold text-slate-600 uppercase tracking-widest">Laporan Baru</h2>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
            @forelse($laporan->where('status', 'baru') as $item)
                @php $cfg = $statusConfig['baru']; @endphp
                <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                    <div class="flex items-center justify-between mb-4">
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 {{ $cfg['bg'] }} border {{ $cfg['border'] }} {{ $cfg['text'] }} text-xs font-semibold rounded-full">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
            @forelse($laporan->where('status', '!=', 'baru') as $item)
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                    ...
                    <a href="{{ route('bk.laporan.show', $item->id) }}"
                        class="flex items-center justify-center gap-1.5 w-full py-2 bg-slate-50 hover:bg-blue-700 text-slate-600 hover:text-white text-sm font-semibold rounded-xl border border-slate-200 hover:border-blue-700 transition-all duration-200">
                        Lihat Detail
                        <i data-feather="arrow-right" class="w-4 h-4"></i>
                    </a>
                </div>
