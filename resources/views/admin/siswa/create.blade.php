...
@section('content')
    ...
    <form action="{{ route('admin.siswa.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-5">
            <div
                class="mb-5 pb-5 border-b border-slate-100 flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="kelas" required
                        ...
                        @foreach(['1', '2', '3'] as $t)
                            @foreach(['A', 'B', 'C'] as $h)
                                @php $nilaiKelas = 'Internasional ' . $t . $h; @endphp
                                <option value="{{ $nilaiKelas }}" {{ old('kelas') === $nilaiKelas ? 'selected' : '' }}>
                                    {{ $nilaiKelas }}
                                </option>
                            @endforeach
                        @endforeach
        ...
        <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl p-6">
                ...
                <form method="POST" action="{{ route('admin.import-siswa.store') }}" enctype="multipart/form-data">
                    ...
                    <div
                        class="border-2 border-dashed border-slate-300 rounded-2xl p-10 text-center bg-slate-50 hover:border-blue-400 hover:bg-blue-50 transition-colors">
                    ...
                    <div class="mt-5 bg-blue-50 border border-blue-100 rounded-2xl px-4 py-3 text-sm text-blue-700">
                        <div class="flex gap-2">
                            ...
                            <p>
                                Urutan kolom boleh acak, yang penting ada
